...
const MovieCard = memo(({ id, poster_path, title, vote_average, release_date, original_language }: Movie) => {
  return (
    <Link href={{ pathname: "/movies/[id]", params: { id: id.toString() } }} asChild>
      <TouchableOpacity className="w-[30%]" activeOpacity={0.8}>
        <ImageBackground
        ...
        <View className="flex-row items-center justify-between">
          <Text className="text-xs text-light-300 font-medium mt-1">{original_language?.toUpperCase()}</Text>
        </View>
      </TouchableOpacity>
    </Link>
  )
